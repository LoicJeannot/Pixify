...
const darkMode = () => {
    const highlightedButtons = document.getElementsByClassName("submitnow_button");
    body.classList.add("dark_mode");
    body.classList.remove("light_mode");
    darkButton.classList.add("hidden")
    lightButton.classList.remove("hidden");
    highlightedButtons[0].style.border = "#7BFFE9";
}
...
const lightMode = () => {
    ...
    highlightedButtons[0].style.border = "none";
}
...
const displayImages = (list) => {
    ...
    const allPictures = list.map((item) => {
       return ` <div class = "image_card">
            <img src = ${item.images}">
            <p>${item.title} -  ${item.name}</p>
        </div>`
    }).join("");
    imageContainer.innerHTML = allPictures;
}
